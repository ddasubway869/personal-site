...
// Returns Unix timestamp (seconds) for Monday 00:00 UTC of the current ISO week
function weekStartUnix(date = new Date()) {
...
function buildEmail({ eyebrow, heading, body, ctaLabel, theme = 'dark', unsubscribeUrl = null }) {
  const t = theme === 'light'
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
          ...
          <tr>
            <td style="padding-top:24px;text-align:center;">
              ...
              <p style="margin:4px 0 0;font-size:11px;color:${t.footer};">You're receiving this because you're a member of ARVL${unsubscribeUrl ? ` · <a href="${unsubscribeUrl}" style="color:${t.footer};text-decoration:underline;">Unsubscribe</a>` : ''}</p>
            </td>
          </tr>
...
async function sendWeeklyOpener() {
  ...
  const members = await db.all(
    `SELECT username, email, unsubscribe_token FROM users WHERE verified = 1 AND email_opt_out = 0 AND created_at < ?`,
    weekStart
  );
  ...
  for (const member of members) {
    // Skip if already sent this week (guards against multiple instances firing)
    const alreadySent = await db.get(
      `SELECT 1 FROM email_logs WHERE type = 'opener' AND recipient = ? AND sent_at >= ?`,
      member.email, weekStart
    );
    if (alreadySent) { console.log(`[scheduler] Opener already sent to ${member.email}, skipping`); continue; }

    const name = member.username || member.email.split('@')[0];
    const unsubscribeUrl = member.unsubscribe_token ? `${BASE_URL}/auth/unsubscribe?token=${member.unsubscribe_token}` : null;
    try {
      await mailer.sendMail({
        from:    `"ARVL" <${process.env.MAIL_FROM}>`,
        to:      member.email,
        subject: 'New Week, New Updates.',
        text:    `Hey ${name},\n\nWeek ${weekNum} is here. What are you listening to?\n\nA couple of things have changed on the site. The home page now shows what's actually resonating across the community over the last 30 days, ranked by picks, saves, and listen-laters combined. Album pages also now show community stats, so you can see exactly how many members have picked, saved, or listen-later'd any album.\n\nIf you haven't checked it out yet, we also launched The Shelf. Save albums to your Collection or Listen Later list and build out your own personal music library right on your profile.\n\nThank you for being part of this. On behalf of the ARVL team, we appreciate every single one of you who shows up each week.\n\n→ ${BASE_URL}\n${unsubscribeUrl ? `\nUnsubscribe: ${unsubscribeUrl}` : ''}`,
        html:    buildEmail({
          eyebrow:  'Weekly pick',
          heading:  `Week ${weekNum} is open.`,
          body:     `What are you listening to this week, ${name}? Submit your pick and see what the community is into.<br><br>A couple of things have changed on the site. The home page now shows what's actually resonating across the community over the last 30 days, ranked by picks, saves, and listen-laters combined. Album pages also now show community stats, so you can see exactly how many members have picked, saved, or listen-later'd any album.<br><br>If you haven't checked it out yet, we also launched The Shelf. Save albums to your Collection or Listen Later list and build out your own personal music library right on your profile.<br><br>Thank you for being part of this. On behalf of the ARVL team, we appreciate every single one of you who shows up each week.`,
...
async function sendNudge() {
  ...
  const weekStart = weekStartUnix();
  const members = await db.all(
    `SELECT u.username, u.email, u.unsubscribe_token
     FROM   users u
     WHERE  u.verified = 1
  ...
  for (const member of members) {
    // Skip if already sent this week (guards against multiple instances firing)
    const alreadySent = await db.get(
      `SELECT 1 FROM email_logs WHERE type = 'nudge' AND recipient = ? AND sent_at >= ?`,
      member.email, weekStart
    );
    if (alreadySent) { console.log(`[scheduler] Nudge already sent to ${member.email}, skipping`); continue; }

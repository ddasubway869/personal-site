...
async function search(query) {
  ...
  const url   = `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=album,artist`;
...
async function getArtistById(artistId) {
  ...
  return {
    spotifyId: a.id,
    name:      a.name,
    imageUrl:  a.images[0]?.url ?? null,   // largest available image
    followers: a.followers?.total ?? 0,
  };
}

/**
 * Get all studio albums for an artist, deduped and sorted oldest → newest.
 */
async function getArtistAlbums(artistId) {
  const token = await getToken();
  const url   = `https://api.spotify.com/v1/artists/${encodeURIComponent(artistId)}/albums?include_groups=album`;
  ...
  const data = await res.json();
  return _filterAndMapAlbums(data.items, true); // newest → oldest
}
...
module.exports = { getToken, search, getArtistById, getArtistAlbums, getAlbum };
